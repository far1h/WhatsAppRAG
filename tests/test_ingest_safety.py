...
class IngestSafetyTest(unittest.TestCase):
    ...
    def test_process_document_sanitizes_text_before_summarizing(self):
        document = {
            "text": "Kei (2025-04-08 18:24): NSFW private detail",
            "metadata": {
                "participants": "Kei, mo",
        ...
        with patch.object(ingest, "summarize_document") as summarize:
            summarize.return_value = ingest.ChunkSummary(
                headline="Private detail",
                summary="A sanitized chat detail.",
            )

            result = ingest.process_document(document)

        summarized_document = summarize.call_args.args[0]
        self.assertNotIn("nsfw", summarized_document["text"].lower())
        self.assertIn("[sensitive term]", result.page_content)
        self.assertTrue(result.metadata["sanitized"])
        self.assertEqual(result.metadata["summary_status"], "generated")
